            ...
            using (var connection = _dbConnectionFactory.CreateConnection())
            {
                var sql = @"
                    SELECT 
                        c.Codigo_Curso,
                        c.Nombre_Curso,
                        CAST(AVG(n.Valor_Nota) AS DECIMAL(5,2)) AS Promedio
                    FROM Estudiante e
                    ...
                    INNER JOIN Rubro r ON g.ID_Grupo = r.ID_Grupo
                    INNER JOIN Nota n ON n.ID_Rubro = r.ID_Rubro AND n.ID_Estudiante = e.ID_Estudiante
